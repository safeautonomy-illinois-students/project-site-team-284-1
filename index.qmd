...
Park-to-drive is capable of driving from an arbitrary parking spot onto the track. Stanley controller is selected as it can account for both cross-track error and heading error at the same time. Once the vehicle joins the track ($(\text{dist\_from\_path} < 2\text{m} \land \text{lane\_confidence} \geq 0.9 \land t \geq 0.5\text{s}) \lor (\text{dist\_from\_path} < 0.5\text{m} \land |\theta_{err}| < 15°)$), control is delegated to the lane follower (pure pursuit-based).
