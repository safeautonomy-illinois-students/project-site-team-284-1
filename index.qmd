...
::: {.results-grid}
::: {.result-card}
![Real-world waypoints generated from GPS data (graph implementation)](static/images/real_waypoint_graph.png){height="300px"}
:::
::: {.result-card}
![Simulation waypoints generated from simulation odometry data (graph implementation)](static/images/sim_waypoint_graph.png){height="300px"}
:::
:::
...
::: {.figure-caption}
$\delta$: steering angle, $e_{heading}$: heading error, $e_{cte}$: cross-track error, $k=0.5$: gain, $v$: speed
:::

$$(\text{dist\_from\_path} < 2\text{m} \land \text{lane\_confidence} \geq 0.9 \land t \geq 0.5\text{s}) \lor (\text{dist\_from\_path} < 0.5\text{m} \land |\theta_{err}| < 15°)$$
::: {.figure-caption}
$\delta$: steering angle, $e_{heading}$: heading error, $e_{cte}$: cross-track error, $k=0.5$: gain, $v$: speed
:::
